/* 동네이야기(피드·뉴스·자료) 공용 순수 헬퍼 — 서버/클라이언트 컴포넌트 양쪽에서 import.
   server-only 의존이 없어 "use client" 파일에서도 안전하게 쓸 수 있다.
   [1006] app/town/shared.tsx 에서 여기(.ts)로 옮겼다 — lib/town/news-list.ts(뉴스룸 행 조립기)가
   이 헬퍼를 쓰는데, 단위 테스트 로더는 .tsx 를 못 읽는다. app/town/shared.tsx 는 재수출만 남겨
   기존 import 경로를 그대로 살린다. */
