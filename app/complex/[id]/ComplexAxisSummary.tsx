...
/* [OPT-48] 단지 허브 2.0 — 워크벤치와 같은 근거(라이브 컨텍스트)를 허브에도 요약.
   ...
   [994] 4칸 숫자 + 신호 줄 → **판단 카드**(구간 · 결론 · 투자 점수 · 핵심 숫자 3(기준일) ·
...
export async function ComplexAxisSummary({
  ...
  /* 대표 수치도 핵심 숫자도 없으면(축 0) 카드가 "판단 보류" 한 줄뿐 — 그건 소음이라 생략 */
  if (!verdict.metric && verdict.numbers.length === 0) return null;

  return (
    <section aria-label="이 단지 판단 카드" className="mt-4 rounded-2xl border border-line bg-surface p-4">
      <div className="flex items-baseline justify-between gap-2">
        <h2 className="text-[13px] font-bold text-ink">이 단지 판단 카드 — 규칙 계산 · AI 진단과 같은 근거</h2>
        <Link
      ...
      <p className="mt-2 t-caption text-text-3">
        참고용 요약이며 투자 권유가 아닙니다 · 숫자마다 기준일·출처 표기 · 표본이 적거나 오래된 축은 그 사실을 함께 적습니다
      </p>
    </section>
  );
}
