...
| 종류 | 보관 위치 | 읽는 쪽 |
|---|---|---|
| Vercel 런타임 키(AUTH_SECRET, SUPABASE_SERVICE_ROLE_KEY, CRON_SECRET, RESEND_API_KEY, AUTH_KAKAO_*, 지도·공공 API 키) | Vercel → Project → Settings → Environment Variables | 서버 코드 |
...
3. 회전(rotation) 주기: 서비스 롤 키·CRON_SECRET·뉴스 수집 시크릿(`automation_secrets.news_ingest` 행) 은 **분기 1회**, 외부 API 키는
...
- 대화에 뉴스 수집 시크릿·테스트 계정 비밀번호가 오간 기록이 있다. 세션은 이를 입력하지 않았지만,
  원칙상 **회전 대상**이다.
  [993 정정] 코드에는 `INGEST_SECRET` 이라는 환경변수가 **없다**(저장소 전수 grep 0건). 뉴스 수집이
  실제로 쓰는 값은 DB 행 `public.automation_secrets` (key = `news_ingest`) 이고, `ingest_daily_news`
  RPC 가 그 행과 대조한다(20260727080008). 따라서 회전은 Vercel 이 아니라 **그 DB 행을 갱신**하고,
  08:00 KST 수집 세션(automation_scripts)이 쓰는 값을 같이 바꾸는 것이다.
