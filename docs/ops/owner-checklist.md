...
| 12 | 뉴스 수집 시크릿(`automation_secrets.news_ingest` DB 행) 회전 여부 | 대화에 값이 오간 적이 있어 원칙상 회전 대상(docs/ops/secrets-policy.md). Vercel 에 `INGEST_SECRET` 이란 변수는 없다 — DB 행과 08:00 수집 세션 값을 함께 교체한다 |
